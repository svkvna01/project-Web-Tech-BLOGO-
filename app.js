...
app.get('/search', requireAuth, async (req, res) => {
    ...
    const sqlTrips = `
        ...
        AND (t.title LIKE ? OR t.description LIKE ? OR l.country LIKE ? OR l.city LIKE ? OR l.activity LIKE ?)
    `;
    ...
    // Zoek in posts
    const sqlPosts = `
        SELECT p.id, u.username, p.caption, p.created_at, u.profilePic
        FROM posts p
    ...
    try {
        const [tripsResults] = await db.query(sqlTrips, [searchTerm, searchTerm, searchTerm, searchTerm, searchTerm]);
        const [usersResults] = await db.query(sqlUsers, [searchTerm, searchTerm, searchTerm, searchTerm]);
        const [postsResults] = await db.query(sqlPosts, [searchTerm]); //HIIIEERRRRR
...
app.get('/trip/:id', requireAuth, async (req, res) => {
    ...
    const currentUser = req.session.user.username;
    const fromProfile = req.query.from === 'profile';//
    const trip = await getTripDetails(tripId);

    const isOwner = trip.username === currentUser;
    res.render('trip', { trip, isOwner, fromProfile });
});

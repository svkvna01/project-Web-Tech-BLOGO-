...
app.post('/planner/create', requireAuth, async (req, res) => {
    try {
        ...
        const { title, description, budget, locations, participants } = req.body;

        // 🔍 DEBUG: Log alle ontvangen data
...
app.post('/planner/update/:tripId', requireAuth, async (req, res) => {
    try {
        ...
        await editTrip(tripId, username, title, description, budget, locations, participants);
